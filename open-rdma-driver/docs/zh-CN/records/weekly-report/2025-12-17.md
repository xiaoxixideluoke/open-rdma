...
**实现方案**：
创建了 RCCL 测试基础设施，核心是 `hack_libc` 内存拦截库。
...
**修复内容**:
修改 `src/memory_proxy_simple.rs` 中的 TCP 客户端逻辑（新增 112 行）：
...
**实现方案**:
修改 `src/verbs/ffi.rs` 和 `src/verbs/core.rs`（新增 108 行，删除 35 行）：
...
1. **内存拦截库**：实现了 `posix_memalign` 和 `hipHostMalloc` 两类关键路径的劫持，确保 RCCL 注册的 MR 都使用 2MB 大页，满足驱动要求
2. **懒加载初始化**：解决了 RCCL 打开所有设备导致的仿真器端口冲突问题，实现了设备与仿真器的一对一映射
3. **连接稳定性**：增加重试机制，支持驱动和仿真器任意顺序启动
